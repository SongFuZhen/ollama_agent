...
// 路径沙箱：所有工具调用必须落在 PROJECT_ROOT 内（含符号链接解析）
// 用 realpath 跟随软链到真实落点，防止 PROJECT_ROOT 内的软链指向外部造成逃逸。
// 目标文件可能尚不存在（如写文件），此时仅对「已存在的父目录」做 realpath 校验。
// 注意：PROJECT_ROOT 自身也需 realpath，否则 macOS 上 /tmp -> /private/tmp 会导致全部误判越界。
let _rootReal = null;
async function rootReal() {
  if (_rootReal === null) _rootReal = await fsp.realpath(PROJECT_ROOT);
  return _rootReal;
}
    ...
    async run({ pattern }) {
      ...
      async function walk(dir) {
        ...
        for (const e of entries) {
          ...
          else if (e.name.includes(pattern)) hits.push(path.relative(PROJECT_ROOT, full));
        }
      }
